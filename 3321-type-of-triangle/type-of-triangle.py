class Solution:
    def triangleType(self, nums: List[int]) -> str:
        ...
        Return a string representing the type of triangle that can be 
        formed or "none" if it cannot form a triangle.  

        Example 1:
        Input: nums = [3,3,3]
        Output: "equilateral"
        Explanation: Since all the sides are of equal length, therefore, 
        it will form an equilateral triangle.
        ...
        if not (nums[0] + nums[1] > nums[2] and nums[1] + nums[2] > nums[0] and nums[2] + nums[0] > nums[1]):
            return "none"
